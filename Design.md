...
- Desktop uses the shadcn sidebar in `icon` collapse mode: 256px expanded and 72px collapsed. Expansion is click-only and persists the preference; the collapsed rail contains centered icons only, with labels available through tooltips, focus, and accessible names. Mobile uses an off-canvas sheet; no workflow depends on hover.
...
- 2026-09-05: Corrected the admin surfaces from screenshot evidence: the login narrative panel must retain dark figure-ground contrast, the Copilot must reserve visible space for its latest response, and the collapsed sidebar is icon-only with no hover expansion.
