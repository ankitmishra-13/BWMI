...
export function AdminAssistant({ admin, applicationId, triggerOnly = false }: { admin: AdminSession; applicationId?: string; triggerOnly?: boolean }) {
  ...
  const body = <div className="flex min-h-0 flex-1 flex-col">
    <Alert className="mx-5 mt-5 bg-secondary/55 sm:mx-6"><ShieldCheck /><AlertTitle>Permission-scoped assistance</AlertTitle><AlertDescription>The Copilot sees only redacted synthetic records available to {admin.name}. It cannot publish or approve changes.</AlertDescription></Alert>
    <ScrollArea className="min-h-0 flex-1 px-5 py-5 sm:px-6">
      <div className="flex flex-col gap-4">
        {!answer && <div className="rounded-xl border bg-card p-5"><span className="grid size-10 place-items-center rounded-lg bg-secondary"><Sparkles className="size-5" /></span><h3 className="mt-4 text-base font-semibold">Start with an operational question</h3><p className="mt-1 text-sm text-muted-foreground">Ask about this permitted queue, region, or application. Recommendations remain review-only.</p></div>}
        {answer && <>
          <div className="ml-auto max-w-[88%] rounded-xl bg-primary px-4 py-3 text-sm text-primary-foreground">{question}</div>
          <div className="max-w-[94%] rounded-xl border bg-card p-5"><div className="flex flex-wrap items-center gap-2"><Bot className="size-5" /><span className="font-semibold">Raahi Ops Copilot</span><Badge variant="outline">{answer.fallback ? 'Built-in analysis' : 'AI-assisted'}</Badge></div><p className="mt-3 whitespace-pre-line text-sm leading-6 text-muted-foreground">{answer.answer}</p><div className="mt-4 border-t pt-3"><p className="text-xs font-semibold uppercase tracking-[.08em] text-muted-foreground">Evidence used</p><ul className="mt-2 flex flex-col gap-1 text-xs text-muted-foreground">{answer.evidence.map((item) => <li key={item} className="flex gap-2"><FileSearch className="mt-0.5 size-3.5 shrink-0" />{item}</li>)}</ul></div></div>
        </>}
    ...
    <div className="border-t bg-card p-5 sm:p-6">
      <div className="mb-3 flex flex-wrap gap-2">{(answer?.suggestions ?? prompts).map((prompt) => <Button key={prompt} type="button" variant="secondary" size="sm" onClick={() => void ask(prompt)}>{prompt}</Button>)}</div>
      <Field><FieldLabel htmlFor={`admin-copilot-${triggerOnly}`}>Ask Raahi Ops Copilot</FieldLabel><Textarea id={`admin-copilot-${triggerOnly}`} value={question} onChange={(event) => setQuestion(event.target.value.slice(0, 300))} maxLength={300} placeholder="Ask about workload, an application, or a citizen update…" className="min-h-24 resize-none" /><FieldDescription>{question.length}/300 · Do not enter real personal data.</FieldDescription></Field>
