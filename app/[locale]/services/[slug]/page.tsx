    ...
    <div lang={locale} className="civic-paper">
      ...
      <main id="main">
        <section className="border-b border-foreground/8 bg-white/64 py-14 sm:py-20">
          <div className="shell">
            <Link href={localPath(locale, `/services?category=${service.category}`)} className="inline-flex min-h-11 items-center gap-2 text-sm font-semibold text-muted-foreground hover:text-foreground"><ArrowLeft className="size-4" />{t(categoryCopy[service.category].title, locale)}</Link>
            <div className="mt-8 grid gap-10 lg:grid-cols-[minmax(0,1.2fr)_minmax(320px,.55fr)] lg:items-start">
              <div className="max-w-4xl"><div className="flex flex-wrap items-center gap-3"><p className="eyebrow">{portal.servicePrototype}</p><Badge variant="outline" className="bg-white">{service.mode === 'transaction' ? (hi ? 'काम करने वाला डेमो' : 'Working demo') : service.mode === 'dashboard' ? (hi ? 'काल्पनिक डैशबोर्ड' : 'Synthetic dashboard') : (hi ? 'जानकारी दृश्य' : 'Information view')}</Badge></div><h1 className="mt-5 text-[clamp(3.2rem,7vw,6.8rem)] leading-[.91] tracking-[-.05em]">{t(service.title, locale)}</h1><p className="mt-6 max-w-3xl text-xl leading-8 text-muted-foreground">{t(service.description, locale)}</p></div>
              <aside className="ios-panel p-6"><dl className="grid grid-cols-2 gap-5"><div><dt className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground"><Clock3 className="size-4" />{portal.expectedTime}</dt><dd className="mt-2 font-semibold">{t(service.duration, locale)}</dd></div><div><dt className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground"><IndianRupee className="size-4" />{portal.sampleFee}</dt><dd className="mt-2 font-semibold">{fee}</dd></div></dl><div className="mt-6 border-t pt-6">{service.mode === 'transaction' ? (user ? <ServiceStartButton locale={locale} serviceSlug={service.slug} label={portal.start} renewalFlow={service.renewalFlow} /> : <Button asChild size="lg"><Link href={demoSignInPath(servicePath(locale, service))}>{portal.signIn}<ArrowRight data-icon="inline-end" /></Link></Button>) : <Button asChild size="lg"><a href="#prototype-view">{service.mode === 'dashboard' ? portal.openDashboard : portal.readGuide}<ArrowRight data-icon="inline-end" /></a></Button>}</div></aside>
            </div>
          </div>
        </section>
        ...
        <section className="py-12 sm:py-16"><div className="shell"><h2 className="text-3xl">{portal.related}</h2><div className="mt-6 grid gap-3 md:grid-cols-3">{related.map((item) => <article key={item.slug} className="ios-panel panel-hover group relative p-6"><h3 className="text-xl"><Link href={servicePath(locale, item)} className="after:absolute after:inset-0">{t(item.title, locale)}</Link></h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{t(item.summary, locale)}</p><span className="mt-4 inline-flex items-center gap-2 text-sm font-semibold">{portal.viewService}<ArrowRight className="size-4 transition-transform group-hover:translate-x-1" /></span></article>)}</div><Button asChild variant="ghost" className="mt-6 px-0"><Link href={localPath(locale, '/services')}><ArrowLeft data-icon="inline-start" />{portal.backServices}</Link></Button></div></section>
      </main>
    </div>
  );
}
...
function SyntheticDashboard({ locale, title }: { locale: 'en' | 'hi'; title: string }) {
  ...
  return <div><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="eyebrow">{locale === 'hi' ? 'पूरी तरह काल्पनिक' : 'Entirely synthetic'}</p><h2 className="mt-2 text-3xl sm:text-5xl">{title}</h2></div><Badge variant="outline" className="w-fit bg-[#FFF9ED] text-warning">SAMPLE DATA</Badge></div><div className="mt-10 grid gap-3 md:grid-cols-4">{values.map((value, index) => <div key={labels[index]} className="ios-panel p-5"><p className="text-sm font-semibold text-muted-foreground">{labels[index]}</p><p className="mt-2 font-heading text-4xl font-semibold">{value}<span className="text-lg text-muted-foreground">%</span></p><div className="mt-4 h-2 rounded-full bg-secondary"><div className="h-full rounded-full bg-primary" style={{ width: `${value}%` }} /></div></div>)}</div><Alert className="mt-10 bg-white"><BarChart3 /><AlertTitle>{locale === 'hi' ? 'ये आधिकारिक आँकड़े नहीं हैं' : 'These are not official statistics'}</AlertTitle><AlertDescription>{locale === 'hi' ? 'चार्ट केवल सुलभ रिपोर्टिंग इंटरफ़ेस दिखाने के लिए बनाया गया है।' : 'The chart exists only to demonstrate an accessible reporting interface.'}</AlertDescription></Alert></div>;
}
...
function InformationGuide({ locale, serviceTitle }: { locale: 'en' | 'hi'; serviceTitle: string }) {
  ...
  return <div className="grid gap-10 lg:grid-cols-[.7fr_1.3fr]"><div><p className="eyebrow">{locale === 'hi' ? 'सरल भाषा' : 'Plain-language guide'}</p><h2 className="mt-3 text-3xl sm:text-5xl">{serviceTitle}</h2><p className="mt-5 text-muted-foreground">{locale === 'hi' ? 'यह अनुभाग लाइव रिकॉर्ड या कानूनी सलाह के बिना सही जानकारी क्रम दिखाता है।' : 'This section demonstrates a useful information order without live records or legal advice.'}</p></div><ol className="border-y">{items.map((item, index) => <li key={item} className="grid grid-cols-[48px_1fr] gap-4 border-b py-6 last:border-0"><span className="text-sm font-semibold text-muted-foreground">0{index + 1}</span><p className="font-medium leading-7">{item}</p></li>)}</ol></div>;
}
