...
export default async function ApplyPage({ params }: { params: Promise<{ locale: string; slug: string; id: string }> }) {
  ...
  return <div lang={locale}><SiteHeader locale={locale} user={user} /><main id="main" className="py-9 sm:py-14"><div className="shell"><p className="eyebrow">{portal.servicePrototype}</p><h1 className="mt-2 max-w-4xl text-3xl font-semibold leading-tight sm:text-5xl">{t(service.title, locale)}</h1><p className="mt-4 max-w-2xl text-[#52667A]">{t(service.summary, locale)}</p><div className="mt-8"><ServiceApplicationWizard initial={application} service={service} locale={locale} /></div></div></main></div>;
}
